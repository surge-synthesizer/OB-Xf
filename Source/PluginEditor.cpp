      ...
      banks (utils.getBankFiles())
{
    ...
    if (juce::PluginHostType().isProTools()) { } else { startTimer(100); }; // Fix ProTools file dialog focus issues
...
void ObxdAudioProcessorEditor::loadSkin (ObxdAudioProcessor& ownerFilter)
{
    ...
    if (voiceSwitch)
    {
        ...
        auto voiceOption = ownerFilter.getValueTreeState().getParameter (ParameterManager::getEngineParameterId (VOICE_COUNT))->getValue();
    ...
    if (legatoSwitch)
    {
        ...
        const auto legatoOption = ownerFilter.getValueTreeState().getParameter (ParameterManager::getEngineParameterId (LEGATOMODE))->getValue();
...
std::unique_ptr<Knob> ObxdAudioProcessorEditor::addKnob(const int x, const int y, const int d, ObxdAudioProcessor &filter,const int parameter,
                                                        const juce::String& /*name*/, const float defval)
{
    ...
    knob->setValue (filter.getValueTreeState().getParameter (ParameterManager::getEngineParameterId (parameter))->getValue());
                                                                     ...
                                                                     const juce::String& name)
{
    ...
    if (parameter != UNLEARN){
        toggleAttachments.add (new juce::AudioProcessorValueTreeState::ButtonAttachment (filter.getValueTreeState(),
                                                                      ParameterManager::getEngineParameterId (parameter),
                                                                      *button));
    } else {
    ...
    button->setToggleState(filter.getValueTreeState().getParameter (ParameterManager::getEngineParameterId (parameter))->getValue(),
...
void ObxdAudioProcessorEditor::actionListenerCallback(const juce::String& message)
{
    ...
    {
        const juce::StringArray parameters{ "pan1Knob", "pan2Knob", "pan3Knob", "pan4Knob", "pan5Knob", "pan6Knob", "pan7Knob", "pan8Knob" };
...
juce::Rectangle<int> ObxdAudioProcessorEditor::transformBounds(int x, int y, int w, int h) const {
    ...
    return juce::Rectangle<int>(x, y, w, h).toFloat().transformedBy(juce::AffineTransform::scale(getScaleFactor())).toNearestInt();
}

juce::ImageButton* ObxdAudioProcessorEditor::addMenuButton (int x, int y, int d, juce::String imgName)
{
    ...
    imageButton->onClick = [this](){
        const juce::ImageButton *imageButton = this->imageButtons[0];
...
void ObxdAudioProcessorEditor::resultFromMenu (const juce::Point<int> pos)
{
    ...
    if (int result = popupMenus[0]->showAt (juce::Rectangle<int> (pos.getX(), pos.getY(), 1, 1)); result >= (skinStart + 1) && result <= (skinStart + skins.size()))
    {
        result -= 1;
        result -= skinStart;
    ...
    else if (result >= (bankStart + 1) && result <= (bankStart + banks.size()))
    {
        ...
        //processor.loadFromFXBFile (bankFile); FIX
    }
...
void ObxdAudioProcessorEditor::MenuActionCallback(int action){
    ...
    {
        if(juce::NativeMessageBox::showOkCancelBox(juce::AlertWindow::NoIcon, "Delete Bank", "Delete current bank: "  /*+processor.currentBank + "?"*/)){
