...
static inline int32_t fxbSwap (const int32_t x) noexcept   { return (int32_t) juce::ByteOrder::swapIfLittleEndian ((uint32_t) x); }

static inline float fxbSwapFloat (const float x) noexcept
{
#ifdef JUCE_LITTLE_ENDIAN
    union { uint32_t asInt; float asFloat; } n;
    n.asFloat = x;
