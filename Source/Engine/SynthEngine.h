...
class SynthEngine
{
  ...
  public:
    ...
    void procAsPlayedAlloc(float val) { synth.asPlayedMode = val > 0.5; }
...
#define OBXF_SRC_ENGINE_SYNTHENGINE_H
    ...
    void processVolume(float param) { synth.Volume = linsc(param, 0, 0.30); }
    ...
    void processPwEnv(float param) { ForEachVoice(pwenvmod = linsc(param, 0, 0.85)); }
    void processPwOfs(float param) { ForEachVoice(pwOfs = linsc(param, 0, 0.75)); }
    void processPwEnvBoth(float param) { ForEachVoice(pwEnvBoth = param > 0.5); }
    void processInvertFenv(float param) { ForEachVoice(invertFenv = param > 0.5); }
    void processPitchModBoth(float param) { ForEachVoice(pitchModBoth = param > 0.5); }
    ...
    void processOversampling(float param) { synth.SetOversample(param > 0.5); }
